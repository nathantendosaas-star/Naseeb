...
export default function LuxuryResidentialPage() {
  const collection = properties.find(p => p.id === 'luxury-apartments');
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white pt-32 pb-24 px-6 md:px-12">
      <SEO 
        title="Luxury Residential Collection | Masembe Real Estate"
        description="Explore our elite collection of luxury apartments and residential landmarks."
      />
          ...
          {collection.gallery.map((src, idx) => (
            ...
            >
              {src.endsWith('.mp4') ? (
                ...
                <OptimizedImage src={src} alt="Residential Gallery" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
              )}
            </motion.div>
          ))}
        </div>
